// =============================================================
// PHASE 4 (FIX) - Strategy A (scatter full-size) + byte-for-byte verification
//   args: model prompt n_pred threads seed cache_mb [verify=0/1]
//   Enters streaming: scatters each expert at absolute offset eid*bytes,
//   KEEPS original router ids (does NOT rewrite to 0..7).
//   verify=1 -> memcmp of each pread against the mmap (PASS/FAIL per layer/eid)
// =============================================================
#include "llama.h"
#include "ggml.h"
#include "gguf.h"
...
static bool g_verify = false;   // arg 7: 1 = checks each pread vs mmap
...
struct Streamer {
  ...
  ExpertCache*cache=nullptr; uint64_t n_expert_reads=0, n_verify_fail=0;
};
...
static void pread_expert(Streamer & s, const ExpertTensor & et,
    ...
    if (g_verify) {
        const uint8_t * ref = (const uint8_t *)et.original + (size_t)eid * et.bytes_per_expert;
        if (memcmp(dst, ref, et.bytes_per_expert) != 0) {
            fprintf(stderr, "VERIFY FAIL: %s eid=%d\n", et.name, eid);
            ++s.n_verify_fail;
        }
    }

#ifdef __linux__
    // cache-purity: manda o kernel descartar essas paginas do page cache,
    // ja que o LRU/Streamer ja guarda os bytes; nao depende do modelo caber na RAM.
    posix_fadvise(s.fd, (off_t)abs, (off_t)et.bytes_per_expert, POSIX_FADV_DONTNEED);
#endif
}
...
static bool moe_callback(struct ggml_tensor*t,bool ask,void*ud){
  ...
  if(s->enable_stream&&has_sub(t->name,"ffn_moe_topk")){
    ...
    // STRATEGY A: full-size buffer (n_expert slots), expert at ABSOLUTE offset
    ...
    auto fill=[&](int kind,int32_t eid,ExpertTensor&et,uint8_t*base){
      ...
      pread_expert(*s,et,eid,dst);
      if(s->cache) s->cache->insert(ck,dst,et.bytes_per_expert);
      ++s->n_expert_reads;
    };

    for(size_t k=0;k<s->n_expert_used;++k){
      int32_t eid=orig_ids[k]; if(eid<0||eid>=(int32_t)s->n_expert) continue;
      fill(0,eid,L.gate,s->gate_full);
      fill(1,eid,L.up,s->up_full);
      fill(2,eid,L.down,s->down_full);
    }

    // STRATEGY A: does NOT rewrite ids -> keeps original eid, ggml reads offset eid*bytes
...
int main(int argc,char**argv){
  ...
  g_verify=argc>7?atoi(argv[7])!=0:0;
  ...
  fprintf(stderr,"%zu layers, %zu experts, top-%zu, cache=%zuMiB, verify=%d\n",n_layer,n_expert,n_used,cache_mb,g_verify);
  ...
  Streamer st; st.gg=gg; st.fd=fd; st.n_expert=n_expert; st.n_expert_used=n_used; st.layers.resize(n_layer);
  ...
  fprintf(stderr,"Phase4-FIX: %zu tokens, %.2fs, %.2f tok/s\n",ng,dt,ng/dt);
